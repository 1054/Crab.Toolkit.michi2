...
if not len(sys.argv) > 2:
    print('Usage: michi2_filter_flux_2sigma_fit_upper_limits.py input_flux.txt output_flux.txt')
    sys.exit()
...
# and fit S/N=1~2 upper limits as f+1*e at all wavelengths
w = data_table.field(data_table.colnames[0])
f = data_table.field(data_table.colnames[1])
ferr = data_table.field(data_table.colnames[2])
mask = (w>=0.0) & (f>1.0*ferr) & (f<=2.0*ferr)
isel = numpy.argwhere(mask).flatten()
if len(isel) > 0:
    for iseli in isel:
...
# 
# and fit S/N<1 upper limits as f+2*e at all wavelengths
w = data_table.field(data_table.colnames[0])
f = data_table.field(data_table.colnames[1])
ferr = data_table.field(data_table.colnames[2])
mask = (w>=0.0) & (f<=1.0*ferr)
isel = numpy.argwhere(mask).flatten()
if len(isel) > 0:
    for iseli in isel:
