...
for (( i = 0; i < ${#list_of_source_names[@]}; i++ )); do
    ...
    if [[ ! -f "fit_2.out" ]]; then
        ...
        michi2-run-fitting-2-components ${list_of_source_redshifts[i]} -flux extracted_flux.txt -parallel 16
    ...
    if [[ ! -f "fit_5.out" ]]; then
        ...
        michi2-run-fitting-5-components ${list_of_source_redshifts[i]} -parallel 16
