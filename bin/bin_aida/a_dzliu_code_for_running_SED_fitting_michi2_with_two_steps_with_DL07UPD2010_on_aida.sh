...
if [[ $# -eq 0 ]]; then
    echo "Usage: "
    echo "    Please input source name! E.g., \"Arp220\""
    echo ""
    exit
fi
...
for (( i = 0; i < ${#list_of_source_names[@]}; i++ )); do
    ...
    if [[ ! -f "fit_5.out" ]]; then
        ...
        michi2-run-fitting-5-components-with-BC03-MultiAge-and-MullaneyAGN-and-DL07UPD2010 ${list_of_source_redshifts[i]} -flux extracted_flux.txt -parallel 8
