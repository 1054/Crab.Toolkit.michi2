...
c          This version :   May 11th, 2015 [reads newSFHs libraries] 
c       ---------------------------------------------------------------------------
...
c       ===========================================================================
       ...
       implicit none
c      character infile(2)*80,outfile*80,user_filt*80 ! dzliu commented original
       character infile(2)*255,outfile*255,user_filt*255 ! dzliu modified 20160720
       integer nmax
...
c      character*10 filt_name(nmax) ! dzliu commented original
       character*29 filt_name(nmax),filt_name_use(nmax) ! dzliu modified 20180319
c      character filter_header*500 ! dzliu commented original
       ...
       real lk_mstar,lh_mstar
       real fmu,ldtot,fbc,aux,lha,lhb,tbc,a_v
...
c       OUTPUT file: catalogue of parameters & magnitudes @ redshift z
       write(numz,'(f6.4)') z
       outfile='starformhist_bc03_z'//numz//'.lbr'
       close (30)
...
c     instead of 10microns - include all the way to 24microns
c     if (lambda_rest(i).lt.10.) then
...
c       Store cosmological parameters	 
     ...
     +  '       A_V',   !25: V-band attenuation
     +  '     LH/M*',   !26: light-to-mass ratio in H-band
     +  '     LK/M*',   !27: light-to-mass ratio in H-band
     +  '    ',filter_header   !28--nfilt_use: model AB magnitudes at redshift z
...
200					format(i10,1pe10.2,0p6f10.4,1p14e10.2,0pf10.4,1p2e10.2,0pf10.4,1p2e10.2,3X,0p,200(f10.4,20X)) ! dzliu 0p50f10.4--> 3X,0p,200(f10.4,20X)
		...
		enddo
		write (*,*)  ' '
		write (*,*)  'Finished reading '//infile(kfile)//'!'
...
c       ===========================================================================
	SUBROUTINE MODEL_AB_COLOR(z,x,ys,y0s,inw,nf,ifilt,wfilt,sfilt,mag)
c       ===========================================================================
...
c       Include attenuation by IGM blueward of Lya
c       Zero flux blueward of Lyman limit (absorption by ISM)
       do i=1,inw
          if (x(i).lt.912.0) then
             ys_new(i)=0.
          else
	     xaux=x(i)
	     call abs_igm_sub(xaux,z,abs_igm)
             ys_new(i)=abs_igm*ys(i)
          endif
       enddo
...
c			F(lambda)*dlambda = F[lambda/(1+z)]*dlambda/(1+z)
			fx(i)=fx(i)/(1.+z)
c			write(*,'(a,i0,a,i0,a,a,a,1pe12.5,a,1pe12.5,a)') 'Compute flux through filter ',i,', filter number ',ifilt(i),', name ',sfilt(i)(1:largo(sfilt(i))),', lambda = ',wfilt(i),' AA, flux = ',fx(i),' Lsun Hz-1 (direct interpolation)' ! dzliu debug
...
c				write(*,'(a,i0,a,i0,a,a,a,1pe12.5,a,1pe12.5,a)') 'Compute flux through filter ',i,', filter number ',ifilt(i),', name ',sfilt(i)(1:largo(sfilt(i))),', lambda = ',wfilt(i),' AA, flux = ',fx(i),' Lsun Hz-1 (applied filter curve)' ! dzliu debug
			endif
		enddo
...
c       ---------------------------------------------------------------------------
        ...
        implicit none
        real*4 lamrest,z,teff,xc,lam,onez,xcoeff
...
c       Effective wavelength of the filter:
	xeff(i)=z1*TRAPZ1(xf,rfc,m)/TRAPZ1(xf,rfd,m) ! obs-frame, AA unit
...
c	take wavelength in detector frame
	xf(m)=xlam(j)*z1 ! obs-frame, AA unit
...
c	DL = Luminosity distance in Mpc
	...
	dismod = 5*alog10(dl(h0,q0,z)*1.E5)
