...
c     observations, filters, etc.
      real*8 w(galmax,nmax),redshift(galmax),dist(nmod),delz
      real*8 zspec(galmax),zmin(galmax),zmax(galmax)
...
c     model libraries, parameters, etc.
      ...
      real*8 fprop_sfh(nmod,nprop_sfh),fmu_sfh(nmod),av(nmod)
      real*8 fprop_ir(nmod,nprop_ir),fmu_ir(nmod)
      real*8 zopt(nmod),zir(nmod),tdust(nmod),f_igm(nmod)
      ...
      real*8 fmu_ism(nmod),mdust(nmod),lmdust(nmod),age(nmod)
      real*8 m_lh(nmod),m_lk(nmod),qir(nmod)
c     chi2, scaling factors, etc.
      real*8 flux_mod(nmax),dist_aux
      real*8 chi2,chi2_sav,chi2_new,df,chi2_nd
...
c     histograms
      ...
      real*8 ml_min,ml_max,dml
      real*8 a_min,a_max,da
      real*8 md_min,md_max,dmd
      real*8 ld_min,ld_max,dldust
      real*8 av_min,av_max,dav
      real*8 tbg1_min,tbg1_max,dtbg,tbg2_min,tbg2_max
      real*8 xi_min,xi_max,dxi,age_min,age_max,dage
      real*8 td_min,td_max,dtd
      real*8 pct_sfr(5),pct_fmu_sfh(5),pct_fmu_ir(5)
      real*8 pct_mu(5),pct_tv(5),pct_mstr(5),pct_z(5)
      real*8 pct_ssfr(5),pct_ld(5),pct_tbg2(5),pct_av(5)
      real*8 pct_tbg1(5),pct_xi1(5),pct_xi2(5),pct_age(5)
      real*8 pct_xi3(5),pct_tvism(5),pct_ism(5),pct_md(5),pct_td(5)
      real*8 pct_m_lh(5),pct_m_lk(5)
      integer nbinmax1,nbinmax2
      parameter (nbinmax1=2000,nbinmax2=200)
      real*8 psfh2(nbinmax2),pir2(nbinmax2),pmu2(nbinmax2)
      real*8 ptv2(nbinmax2),pxi2_2(nbinmax2),pssfr2(nbinmax2)
      real*8 pa2(nbinmax2),pldust2(nbinmax2),pav_2(nbinmax2)
      ...
      real*8 tvism2_hist(nbinmax2),av2_hist(nbinmax2),z2_hist(nbinmax2)
      real*8 td2_hist(nbinmax2),age2_hist(nbinmax2)
      real*8 ml2_hist(nbinmax2),pm_lh2(nbinmax2),pm_lk2(nbinmax2)
      real*8 pz_2(nbinmax2),page_2(nbinmax2),ptd_2(nbinmax2)
      real*8 pmstar_av(nbinmax1,nbinmax1),pmstar_av2(nbinmax2,nbinmax2)
      real*8 pav_z(nbinmax1,nbinmax1),pav_z2(nbinmax2,nbinmax2)
      real*8 pav_age(nbinmax1,nbinmax1),pav_age2(nbinmax2,nbinmax2)
      real*8 pz_td(nbinmax1,nbinmax1),pz_td2(nbinmax2,nbinmax2)
      real*8 pm_lh(nbinmax1),pm_lk(nbinmax1),alpha
      real*8 pav_mlh(nbinmax1,nbinmax1),pav_mlh2(nbinmax2,nbinmax2)
      real*8 page_mlh(nbinmax1,nbinmax1),page_mlh2(nbinmax2,nbinmax2)
      real*8 sf_min,sf_max,dsf,psf_aux(nbinmax1),pct_sf_aux(5),pct_sf(10,5)
      integer nbin_fmu,nbin_mu,nbin_tv,nbin_a,nbin2_tvism
      integer nbin_tbg1,nbin_tbg2,nbin_xi,nbin_sfr,nbin_ld,nbin_age
      integer nbin2_fmu,nbin2_mu,nbin2_tv,nbin2_a,nbin_fmu_ism,nbin_td
      integer nbin2_fmu_ism,nbin_md,nbin2_md,nbin_ssfr,nbin2_ssfr
      integer nbin2_tbg1,nbin2_tbg2,nbin2_xi,nbin2_sfr,nbin2_ld
      integer nbin_z,nbin_av,nbin2_av,nbin2_z,nbin2_age,nbin2_td
      integer nbin_ml,nbin2_ml,nbin_sf
      real*8 fmu_hist(nbinmax1),psfh(nbinmax1),pism(nbinmax1)
      real*8 pir(nbinmax1),ptbg1(nbinmax1),pz(nbinmax1),z_hist(nbinmax1)
      real*8 z_min,z_max,dz,aux1,aux2,ptd(nbinmax1),ml_hist(nbinmax1)
      ...
      real*8 pssfr(nbinmax1),a_hist(nbinmax1),pa(nbinmax1),td_hist(nbinmax1)
      real*8 ld_hist(nbinmax1),pldust(nbinmax1),pav(nbinmax1),age_hist(nbinmax1)
      real*8 tbg1_hist(nbinmax1),tbg2_hist(nbinmax1),av_hist(nbinmax1)
      real*8 ssfr_hist(nbinmax1),xi_hist(nbinmax1),pxi1(nbinmax1)
      real*8 pxi2(nbinmax1),pxi3(nbinmax1),page(nbinmax1)
      real*8 sf_hist(nbinmax1),psf(10,nbinmax1)
      real*8 md_hist(nbinmax1),pmd(nbinmax1),i_td(nmod)
      real*8 i_fmu_sfh(nmod),i_fmu_ir(nmod),i_age(nmod)
      real*8 i_mu(nmod),i_tauv(nmod),i_tvism(nmod)
      real*8 i_lssfr(nmod),i_fmu_ism(nmod),i_ml_h(nmod),i_ml_k(nmod)
      real*8 i_tbg1(nmod),i_xi1(nmod),i_xi2(nmod),i_xi3(nmod)
      real*8 i_tbg2(nmod),i_z(nmod),i_av(nmod),sfh(10,nmod),i_sf(10,nmod)
...
c     dzliu modified: data tbg2_min/15./,tbg2_max/25.0125/,dtbg/0.025/ -> tbg2_max/55.0125/,dtbg/0.05/
      ...
      data ml_min/-1./,ml_max/4./,dml/0.005/
      data sf_min/-15./,sf_max/-5./,dsf/0.01/
      data tcenter/5.5,6.5,7.35,7.85,8.35,8.85,9.20,9.55,9.85,10.1/
      data tmin/5.,6.,7.,7.70,8.,8.70,9.,9.40,9.70,10./
      data tmax/6.,7.,7.7,8.,8.7,9.,9.4,9.7,10.,10.13/
c     cosmology
...
c     fit(ifilt)=0: do not fit flux from filter ifilt (set flux=-99)
      do ifilt=1,nfilt
            flux_orig(i_gal,ifilt)=flux_obs(i_gal,ifilt)
            sigma_orig(i_gal,ifilt)=sigma(i_gal,ifilt)
      enddo
...
c     - 2.5 mic < lambda(rest-frame) < 30 mic : stellar + dust emission
c     - lambda(rest-frame) > 30 mic : emission purely from dust
c     ---------------------------------------------------------------------------
         ...
         do i=1,nfilt
            ...
            if (lambda_rest(i).gt.2.5.and.lambda_rest(i).le.30) then
         ...
         write(*,*) '   '
         write(*,*) 'debug dzliu: **************************************************************'
         write(*,*) 'debug dzliu: Input data point number nfilt ', nfilt
         write(*,*) 'debug dzliu: Stellar data points nfilt_sfh ', nfilt_sfh
         write(*,*) 'debug dzliu: Interm. data points nfilt_mix ', nfilt_mix
         write(*,*) 'debug dzliu: Infrared data points nfilt_ir ', nfilt_ir
         write(*,*) 'debug dzliu: **************************************************************'
...
c     READ OPTLIB
            ...
            do while(io.eq.0)
               i_sfh=i_sfh+1
               read(21,*,iostat=io) indx_sfh(i_sfh),(fprop_sfh(i_sfh,j),j=1,nprop_sfh),
     +              (flux_sfh(i_sfh,j),j=1,nfilt_sfh)
               if (io.eq.0) then
c     Relevant physical parameters
                  fmu_sfh(i_sfh)=fprop_sfh(i_sfh,22)            ! fmu parameter Ld(ISM)/Ld(tot) - optical ! dzliu note: see 'fprop_sfh' column descriptions in "get_optic_colors_highz.f". 
                  mstr1(i_sfh)=fprop_sfh(i_sfh,6)               ! stellar mass
                  ldust(i_sfh)=fprop_sfh(i_sfh,21)/mstr1(i_sfh) ! total luminosity of dust (normalize to Mstar) ! dzliu note: this is the code fixing L_dust to L_star.
...
c         do k=1,nfilt
c            write(*,*) k, filt_name(k),flux_obs(i_gal,k), sigma(i_gal,k)
c            enddo
c            stop
...
c     Observed fluxes: Jy -> Lsun/Hz   ! dzliu note: redshift to restframe, dist()**2 already contains a factor of 1/(1+z)
     ...
     +              *3.283608731e-33*(dist(i_gal)**2)
               ...
               flux_orig(i_gal,k)=flux_orig(i_gal,k)*1.e-23
     +              *3.283608731e-33*(dist(i_gal)**2)
               sigma_orig(i_gal,k)=sigma_orig(i_gal,k)*1.e-23
     +              *3.283608731e-33*(dist(i_gal)**2)
            if (sigma(i_gal,k).lt.0.05*flux_obs(i_gal,k)) then
               sigma(i_gal,k)=0.05*flux_obs(i_gal,k)
            endif
c     upper limits: obs flux set to -99. and sigma=upper limit value
c     set the flux to be ~0 (1.e-6 times sigma because zero won't do)
            if (flux_obs(i_gal,k).lt.0.and.sigma(i_gal,k).gt.0) then
               sigma(i_gal,k)=sigma(i_gal,k)*1.e-23
     +              *3.283608731e-33*(dist(i_gal)**2)
...
c     average Tdust
         call get_histgrid(dtd,td_min,td_max,nbin_td,td_hist)
...
c     [makes code faster -- implemented by the Nottingham people]
         do i_sfh=1, n_sfh
            ...
            aux=((av(i_sfh)-av_min)/(av_max-av_min)) * nbin_av
            i_av(i_sfh) = 1 + dint(aux)
            ...
            aux=((m_lk(i_sfh)-ml_min)/(ml_max-ml_min))* nbin_ml
            i_ml_k(i_sfh) = 1 + dint(aux)
         ...
         do i_ir=1, n_ir
            ...
            aux=((tdust(i_ir)-td_min)/(td_max-td_min))* nbin_td
            i_td(i_ir) = 1+dint(aux)
...
c     Compute scaling factor "a" - this is the number that minimizes chi^2
                  do k=1,nfilt
                     if (flux_obs(i_gal,k).gt.0) then
                        num=num+(flux_mod(k)*flux_obs(i_gal,k)*w(i_gal,k))   ! dzliu note: w(i_gal,k) is the 1/sigma^2 weighting
...
c     mass-weighted age
                   ibin= i_age(i_sfh)
                  ibin = max(1,min(ibin,nbin_age))
                  page(ibin)=page(ibin)+prob

c     mass-to-light ratio (H-band)
                  ibin=i_ml_h(i_sfh)
                  ibin = max(1,min(ibin,nbin_ml))
                  pm_lh(ibin)=pm_lh(ibin)+prob
c     mass-to-light ratio (K-band)
                  ibin=i_ml_k(i_sfh)
                  ibin = max(1,min(ibin,nbin_ml))
...
c    average Tdust
                  ibin= i_td(i_ir)
                  ibin = max(1,min(ibin,nbin_td))
                  ptd(ibin)=ptd(ibin)+prob
c     xi_PAH^tot
                  ibin= i_xi1(i_ir)
...
c     ---------------------------------------------------------------------------
         ...
         call get_percentiles(nbin_av,av_hist,pav,pct_av)
         ...
         call get_percentiles(nbin_td,td_hist,ptd,pct_td)
         ...
         call get_percentiles(nbin_age,age_hist,page,pct_age)
         call get_percentiles(nbin_ml,ml_hist,pm_lh,pct_m_lh)
         call get_percentiles(nbin_ml,ml_hist,pm_lk,pct_m_lk)
...
c     New histogram parameters
     ...
     +        tv2_hist,ptv,ptv2)
         call degrade_hist(dav,av_min,av_max,nbin_av,nbin2_av,av_hist,
     +        av2_hist,pav,pav_2)
     ...
     +        tbg2_hist,tbg2_2_hist,ptbg2,ptbg2_2)
         call degrade_hist(dtd,td_min,td_max,nbin_td,
     +        nbin2_td,
     +        td_hist,td2_hist,ptd,ptd_2)
     ...
     +        md_hist,md2_hist,pmd,pmd_2)
         call degrade_hist(dage,age_min,age_max,nbin_age,nbin2_age,
     +        age_hist,age2_hist,page,page_2)
         call degrade_hist(dml,ml_min,ml_max,nbin_ml,nbin2_ml,
     +        ml_hist,ml2_hist,pm_lh,pm_lh2)
         call degrade_hist(dml,ml_min,ml_max,nbin_ml,nbin2_ml,
     +        ml_hist,ml2_hist,pm_lk,pm_lk2)
...
c     ---------------------------------------------------------------------------
 ...
 802     format('# fmu(SFH)...fmu(IR)........mu......tauv',
     ...
     +        '....lg(M/Lh)....lg(M/Lk)') 						       ! dzliu modified, align with headers, added ".." before "tvism", "...." before "SFR", "..." before "A_V", 
 803     format(0p4f10.3,1p3e12.3,0p2f12.1,0p5f12.3,1p2e12.3,0p8f12.3) ! dzliu modified, align with headers
         
         write(31,703)
         write(31,804)
 804     format('# BEST FIT MODEL: (i_sfh, i_ir, chi2, redshift)')        ! dzliu note: a_sav is just M*
         write(31,311) indx_sfh(sfh_sav),indx_ir(ir_sav),chi2_sav/n_flux,
     +        redshift(i_gal)
 311     format(2i10,0pf10.3,0pf12.6)
     ...
     +        k=nfilt_sfh+1,nfilt)
 701     format(1p,200(e-29.3,' ')) ! dzliu modified, add space between values, and 50(e12.3) --> 200(e12.3)
 ...
 889     format('# ... SFR_0.1Gyr ...')
         ...
         write(31,890)
 890     format('# ... age_M (mass-weighted age) ...')
         do ibin=1,nbin2_age
            write(31,812) age2_hist(ibin),page_2(ibin)
         enddo
         write(31,60)
         write(31,61) (pct_age(k),k=1,5)

         write(31,891)
 891     format('# ... log(Mstar/L_H) ...')
         do ibin=1,nbin2_ml
            write(31,812) ml2_hist(ibin),pm_lh2(ibin)
         enddo
         write(31,60)
         write(31,61) (pct_m_lh(k),k=1,5)

         write(31,892)
 892     format('# ... log(Mstar/L_K) ...')
         do ibin=1,nbin2_ml
            write(31,812) ml2_hist(ibin),pm_lk2(ibin)
         enddo
         write(31,60)
         write(31,61) (pct_m_lk(k),k=1,5)

         write(31,776)
 776     format('# ... A_V ...')
         do ibin=1,nbin2_av
            write(31,807) av2_hist(ibin),pav_2(ibin)
         enddo
         write(31,60)
         write(31,61) (pct_av(k),k=1,5)

         write(31,771)
 771     format('# ... Tdust ...')
         do ibin=1,nbin2_td
            write(31,807) td2_hist(ibin),ptd_2(ibin)
         enddo
         write(31,60)
         write(31,61) (pct_td(k),k=1,5)

         write(*,*) 'Storing best-fit SED...'
         write(*,*) outfile2

            call get_bestfit_sed(indx_sfh(sfh_sav),indx_ir(ir_sav),a_sav,
...
c     character*10 outfile ! dzliu modified
      ...
      parameter(niw_tot=13117)
      real tform,gamma,zmet,tauv0,mu,mstr1,mstr0,mstry,tlastburst,age_wm,age_wr
      real fmu,ldtot,fbc,aux,lha,lhb,xaux
      real wl_opt(6918),opt_sed(6918),opt_sed0(6918)
      real wl_ir(6750),ir_sed(6750),irprop(9)
      real age(1000),sfr(1000),nu(niw_tot)
      real fburst(5),ftot(5),sfrav(5),av,tbc,m_lh,m_lk
      real*8 dfmu_aux,dmstar,dz,ddist,ff_igm
      real fmu_aux,mstar,z,dist,irlums(3)
      real fmuopt(50000),fmuir(50000),abs_igm,f_igm
      real fopt(6918),fopt0(6918),ldust
      real fir(6750),fopt_int(niw_tot),fopt_int0(niw_tot)
...
c     Read Optical SEDs and properties (1st file = models 1 to 25000)
 7    read (29) niw_opt,(wl_opt(i),i=1,niw_opt)
     ...
     .        lha,lhb,aux,aux,ldtot,fmu,fbc,tbc,av,m_lh,m_lk
...
c     Read Infrared SEDs and properties
 ...
 317  format('#...xi_C^ISM....T_W^BC/K...T_C^ISM/K...xi_PAH^BC',
     +     '...xi_MIR^BC.....xi_W^BC....Mdust/Mo........q_IR')
      write(31,318) (irprop(k),k=2,7),irprop(8)*ldust*mstar,irprop(9)
 318  format(0p6f12.3,1pe12.3,0pf12.3)
...
c     Final SEDs - write output file
      do i=1,niw_tot-1
         ...
         if (wl(i).lt.912.0) then !add IGM absorption to the final SEDs
            sedtot(i)=-99.
            fopt_new0(i)=-99.
            else
               xaux=wl(i)
               call abs_igm_sub(xaux,z,abs_igm)
               sedtot(i)=abs_igm*sedtot(i)
               fopt_new0(i)=abs_igm*fopt_new0(i)
            endif
            wl(i)=log10(wl(i)*(1.+z))               ! dzliu note: redshift SED library wavelength to the obsframe by *(1+z)
            sedtot(i)=log10(sedtot(i)/(1.+z))       ! dzliu note: redshift SED library flux Lsun AA-1 to the obsframe by /(1+z)
            fopt_new0(i)=log10(fopt_new0(i)/(1.+z))
            aux=sedtot(i)
            if (aux.ne.aux) then
               sedtot(i)=-99.
               fopt_new0(i)=-99.
            endif
              write(31,311) wl(i),sedtot(i),fopt_new0(i)
      enddo
 311  format(3f12.6)
...
c       ===========================================================================
	SUBROUTINE ABS_IGM_SUB(lamrest,z,xcoeff)
c       ===========================================================================
c       From Tamas Budavari (02/2004): Madau (1995) IGM attenuation prescription
c       Returns flux attenuation factor. Includes more coefficients (and hence is
c       more precise) than actually published formulae.
c       ---------------------------------------------------------------------------

        implicit none
        real*4 lamrest,z,teff,xc,lam,onez,xcoeff,factor
