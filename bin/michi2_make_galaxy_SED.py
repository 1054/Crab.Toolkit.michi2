...
Lib_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))+os.sep+'data'+os.sep+'make_lib_SED'+os.sep+'02_Make_Lib_SED'
...
def find_SED_lib_by_given_parameters(Lib_file_path, Lib_params_dict, verbose = True): 
    ...
    # Lib_file_path = Lib_dir+os.sep+Lib_file
...
if numpy.isnan(z) or (numpy.isnan(Mstar) and numpy.isnan(LIR)):
    print('Error! Please check the about inputs!')
    sys.exit()
...
if ~numpy.isnan(Umin):
    ...
    if numpy.isnan(Umean):
        Umean = 3.0 * (1.0+z)**1.80
    Temp_Grid_Umin = numpy.arange(0.01,50.0+0.01,0.01)
    Temp_Grid_Umean = (1-fPDR) * Temp_Grid_Umin + fPDR * Temp_Grid_Umin * (numpy.log(1e6/Temp_Grid_Umin)/(1-Temp_Grid_Umin/1e6))
...
if numpy.isnan(qPAH):
    qPAH = 0.47
if numpy.isnan(qIR):
    qIR = 2.35*(1+(z))**(-0.12) + numpy.log10(1.91)
    # 
    # De Haize et al. 2017
...
Lib_stellar = 'BC03_Constant_SFH_various_ages/lib.BC03.Padova1994.BaSeL.Z0.0190.EBV.SED'
Lib_AGN = 'lib.MullaneyAGN.SED'
Lib_warm_dust = 'lib.DL07.HiExCom.SED'
Lib_cold_dust = 'lib.DL07.LoExCom.SED'
Lib_radio = 'lib.RadioPowerlaw.Single.SED'
...
if ~numpy.isnan(Mstar):
    if numpy.isnan(Age):
        w_stellar, f_stellar, p_stellar = find_SED_lib_by_given_parameters(Lib_dir+os.sep+Lib_stellar, {'EBV':EBV}, verbose = False)
    else:
        #w_stellar, f_stellar, p_stellar = find_SED_lib_by_given_parameters(Lib_dir+os.sep+Lib_stellar, {'EBV':EBV, 'Age':Age/1e9}, verbose = True)
        # 
...
if fAGN > 0.0:
    w_AGN, f_AGN, p_AGN = find_SED_lib_by_given_parameters(Lib_dir+os.sep+Lib_AGN, {'AGN_TYPE':TAGN}, verbose = False)
...
w_warm_dust, f_warm_dust, p_warm_dust = find_SED_lib_by_given_parameters(Lib_dir+os.sep+Lib_warm_dust, {'Umin':Umin, 'qPAH':qPAH}, verbose = False)
w_cold_dust, f_cold_dust, p_cold_dust = find_SED_lib_by_given_parameters(Lib_dir+os.sep+Lib_cold_dust, {'Umin':Umin, 'qPAH':qPAH}, verbose = False)
# 
# since
...
w_radio, f_radio, p_radio = find_SED_lib_by_given_parameters(Lib_dir+os.sep+Lib_radio, {'POWER_INDEX':0.8}, verbose = False) # The radio SED are normalized to f20cm(rest-frame)=1mJy
