...
if not len(sys.argv) > 2:
    print('Usage: michi2_filter_flux_2sigma_no_stellar_no_radio_fit_infrared_upper_limits.py input_flux.txt output_flux.txt')
    sys.exit()
...
mask = (w>10000) | (w<20) | (ferr>=1e10) #<20180814> fixed bug, added "| (ferr>=1e10)"
isel = numpy.argwhere(mask).flatten()
if len(isel) > 0:
    data_table.remove_rows(isel)

# 
# exclude S/N<3 data at optical
w = data_table.field(data_table.colnames[0])
f = data_table.field(data_table.colnames[1])
ferr = data_table.field(data_table.colnames[2])
...
# and fit S/N=1~2 upper limits as f+1*e at infrared
w = data_table.field(data_table.colnames[0])
f = data_table.field(data_table.colnames[1])
ferr = data_table.field(data_table.colnames[2])
mask = (w>=8.0) & (f>1.0*ferr) & (f<=2.0*ferr)
isel = numpy.argwhere(mask).flatten()
if len(isel) > 0:
    for iseli in isel:
        f[iseli] = f[iseli]+1.0*ferr[iseli]
        print('Fitting upper limit at row %d w %s as f+1*e +- e: %s +- %s'%(iseli, w[iseli], f[iseli], ferr[iseli]))
# 
# and fit S/N<1 upper limits as f+2*e at infrared
w = data_table.field(data_table.colnames[0])
f = data_table.field(data_table.colnames[1])
ferr = data_table.field(data_table.colnames[2])
mask = (w>=8.0) & (f<=1.0*ferr)
isel = numpy.argwhere(mask).flatten()
if len(isel) > 0:
    for iseli in isel:
        f[iseli] = f[iseli]+2.0*ferr[iseli]
        print('Fitting upper limit at row %d w %s as f+2*e +- e: %s +- %s'%(iseli, w[iseli], f[iseli], ferr[iseli]))
