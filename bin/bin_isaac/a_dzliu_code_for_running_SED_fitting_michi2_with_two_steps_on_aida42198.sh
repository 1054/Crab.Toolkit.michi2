...
for (( i = 0; i < ${#list_of_source_names[@]}; i++ )); do
    ...
    #    #michi2-deploy-files
    #    
    #    michi2-run-fitting-2-components-with-BC03-MultiAge-and-MullaneyAGN ${list_of_source_redshifts[i]} -flux extracted_flux.txt -parallel 8
    #    
    ...
    if [[ ! -f "fit_5.out" ]]; then
        ...
        michi2-run-fitting-5-components ${list_of_source_redshifts[i]} -flux extracted_flux.txt -parallel 8
