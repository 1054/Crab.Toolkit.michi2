...
if not len(sys.argv) > 2:
    print('Usage: michi2_filter_flux_2sigma_with_SNR_limit_20.py input_flux.txt output_flux.txt')
    sys.exit()
...
# 
# exclude S/N<3 data at optical/near-infrared
w = data_table.field(data_table.colnames[0])
f = data_table.field(data_table.colnames[1])
ferr = data_table.field(data_table.colnames[2])
...
# 
# exclude S/N<=1 data at far-infrared/millimeter/radio
w = data_table.field(data_table.colnames[0])
f = data_table.field(data_table.colnames[1])
ferr = data_table.field(data_table.colnames[2])
mask = (w>=8.0) & (f<=1*ferr)
...
# and fit S/N=1~2 upper limits as f+1*e at far-infrared/millimeter/radio
w = data_table.field(data_table.colnames[0])
f = data_table.field(data_table.colnames[1])
ferr = data_table.field(data_table.colnames[2])
...
# limit optical/near-infrared S/N to be not larger than 10
w = data_table.field(data_table.colnames[0])
f = data_table.field(data_table.colnames[1])
ferr = data_table.field(data_table.colnames[2])
mask = (w<8.0) & (ferr<f/10.0)
isel = numpy.argwhere(mask).flatten()
if len(isel) > 0:
