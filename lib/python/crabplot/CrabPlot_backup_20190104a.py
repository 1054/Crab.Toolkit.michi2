...
class CrabPlot(object):
    ...
    # 
    # def set_xcharsize
    def set_xcharsize(self, ax=None, panel=None, charsize=None, minortickscharsize=None):
        if len(self.Plot_panels)>0:
            if ax is None:
                if panel is None:
                    panel = len(self.Plot_panels)
    ...
    # 
    # def set_ycharsize
    def set_ycharsize(self, ax=None, panel=None, charsize=None, minortickscharsize=None):
        if len(self.Plot_panels)>0:
            if ax is None:
                if panel is None:
                    panel = len(self.Plot_panels)
        ...
        if x1 is not None and y1 is not None:
            ...
            if plot_panel_ax:
                if NormalizedCoordinate is True:
                    ...
                    plot_one_line = matplotlib.lines.Line2D([x0,x1], [y0,y1], linestyle=linestyle, linewidth=linewidth, transform=plot_panel_ax.transAxes, **kwargs)
                else:
                    plot_one_line = matplotlib.lines.Line2D([x0,x1], [y0,y1], linestyle=linestyle, linewidth=linewidth, **kwargs)
                # 
            ...
            if plot_panel_ax:
                if NormalizedCoordinate is True:
                    ...
                    #x0_for_plot = x0 * (xrange_for_plot[1]-xrange_for_plot[0]) + xrange_for_plot[0]
                    #y0_for_plot = y0 * (yrange_for_plot[1]-yrange_for_plot[0]) + yrange_for_plot[0]
                    plot_one_line = matplotlib.lines.Line2D(x0, y0, linestyle=linestyle, linewidth=linewidth, transform=plot_panel_ax.transAxes, **kwargs)
                else:
                    plot_one_line = matplotlib.lines.Line2D(x0, y0, linestyle=linestyle, linewidth=linewidth, **kwargs)
                # 
                plot_panel_ax.add_line(plot_one_line)
        # 
