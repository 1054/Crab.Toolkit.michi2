...
def test_2():
    # 
    # if we generate the SED model at z=3, then how to scale to z=5?
    # 
    z = 3.0
    ...
    wavelength = SED_dict['lambda_obs_um']
    Snu_obs = SED_dict['Snu_obs_mJy']
    
    z2 = 5.0
    dL2 = cosmo.luminosity_distance(z2).to(u.Mpc).value
    wavelength2 = SED_dict['lambda_rest_um'] * (1.+z2)
    Snu_obs2 = SED_dict['Snu_rest_mJy'] * dL**2 / dL2**2 * (1.+z2)
    
    z3 = 1.5
    dL3 = cosmo.luminosity_distance(z3).to(u.Mpc).value
    wavelength3 = SED_dict['lambda_rest_um'] * (1.+z3)
    Snu_obs3 = SED_dict['Snu_rest_mJy'] * dL**2 / dL3**2 * (1.+z3)
    ...
    ax1.plot(wavelength, Snu_obs+0.0002, c='orange')
    ax1.plot(wavelength2, Snu_obs2+0.0002, c='blue', alpha=0.6)
    ax1.plot(wavelength3, Snu_obs3+0.0002, c='green', alpha=0.6)
    ...
    ax1.text(0.04, 0.90, r'$z=%0.2f$'%(z), transform=ax1.transAxes, fontsize=14, color='orange')
    ax1.text(0.04, 0.80, r'$z2=%0.2f$'%(z2), transform=ax1.transAxes, fontsize=14, color='blue')
    ax1.text(0.04, 0.70, r'$z3=%0.2f$'%(z3), transform=ax1.transAxes, fontsize=14, color='green')
    
    freq_rest = 2.99792458e5/(SED_dict['lambda_rest_um'])
    Snu_obs = SED_dict['Snu_rest_mJy'] * (1.+z)
    freq_rest = freq_rest[::-1]
    Snu_obs = Snu_obs[::-1]
    
    freq_rest2 = 2.99792458e5/(SED_dict['lambda_rest_um'])
    Snu_obs2 = SED_dict['Snu_rest_mJy'] * dL**2 / dL2**2 * (1.+z2)
    freq_rest2 = freq_rest2[::-1]
    Snu_obs2 = Snu_obs2[::-1]
    
    freq_rest3 = 2.99792458e5/(SED_dict['lambda_rest_um'])
    Snu_obs3 = SED_dict['Snu_rest_mJy'] * dL**2 / dL3**2 * (1.+z3)
    freq_rest3 = freq_rest3[::-1]
    Snu_obs3 = Snu_obs3[::-1]
    
    linefreq = 115.2712018; linelabel = r'CO(1-0)'
    ax2 = fig.add_subplot(grid[2,0])
    ax2.plot(freq_rest, Snu_obs, c='orange')
    ax2.plot(freq_rest2, Snu_obs2, c='blue', alpha=0.6)
    ax2.plot(freq_rest3, Snu_obs3, c='green', alpha=0.6)
    ax2.set_ylabel(r'Flux Density [$\mathrm{mJy}$]')
    ax2.set_xscale('log')
    ax2.set_xlim(np.array([linefreq-2.0,linefreq+2.2]))
    ax2.set_ylim([0.0, 1.8 * np.max(Snu_obs[ np.logical_and(freq_rest > (ax2.get_xlim()[0]), freq_rest < (ax2.get_xlim()[1]) ) ])])
    ax2.set_xticks(np.arange(linefreq-2.0,linefreq+2.2,1.0))
    ax2.text(0.04, 0.95, linelabel, va='top', transform=ax2.transAxes, fontsize=12)
    ...
    linefreq = 230.538; linelabel = r'CO(2-1)'
    linefreq = 691.4730763; linelabel = r'CO(6-5)'
    ax3 = fig.add_subplot(grid[2,1])
    ax3.plot(freq_rest, Snu_obs, c='orange')
    ax3.plot(freq_rest2, Snu_obs2, c='blue', alpha=0.6)
    ax3.set_xscale('log')
    ax3.set_xlim(np.array([linefreq-2.0,linefreq+2.2]))
    ax3.set_ylim([0.0, 1.8 * np.max(Snu_obs[ np.logical_and(freq_rest > (ax3.get_xlim()[0]), freq_rest < (ax3.get_xlim()[1]) ) ])])
    ax3.set_xticks(np.arange(linefreq-2.0,linefreq+2.2,1.0))
    ax3.text(0.04, 0.95, linelabel, va='top', transform=ax3.transAxes, fontsize=12)
    ...
    linefreq = 806.6518060; linelabel = r'CO(7-6)+[CI](2-1)'
    ax4 = fig.add_subplot(grid[2,2])
    ax4.plot(freq_rest, Snu_obs, c='orange')
    ax4.plot(freq_rest2, Snu_obs2, c='blue', alpha=0.6)
    ax4.set_xscale('log')
    ax4.set_xlim(np.array([linefreq-4.0,linefreq+4.2]))
    ax4.set_ylim([0.0, 1.8 * np.max(Snu_obs[ np.logical_and(freq_rest > (ax4.get_xlim()[0]), freq_rest < (ax4.get_xlim()[1]) ) ])])
    ax4.set_xticks(np.arange(linefreq-4.0,linefreq+4.2,2.0))
    ax4.text(0.04, 0.95, linelabel, va='top', transform=ax4.transAxes, fontsize=11)
    ...
    linefreq = 1151.9854520; linelabel = r'[CO](10-9)+H2O'
    ax5 = fig.add_subplot(grid[2,3])
    ax5.plot(freq_rest, Snu_obs, c='orange')
    ax5.plot(freq_rest2, Snu_obs2, c='blue', alpha=0.6)
    ax5.set_xscale('log')
    ax5.set_xlim(np.array([linefreq-4.0,linefreq+4.2]))
    ax5.set_ylim([0.0, 1.8 * np.max(Snu_obs[ np.logical_and(freq_rest > (ax5.get_xlim()[0]), freq_rest < (ax5.get_xlim()[1]) ) ])])
    ax5.set_xticks(np.arange(linefreq-4.0,linefreq+4.2,4.0))
    ax5.text(0.04, 0.95, linelabel, va='top', transform=ax5.transAxes, fontsize=12)
    ...
    ax6.plot(freq_rest, Snu_obs, c='orange')
    ax6.plot(freq_rest2, Snu_obs2, c='blue', alpha=0.6)
    ax6.set_xscale('log')
    ...
    ax7.plot(freq_rest, Snu_obs, c='orange')
    ax7.plot(freq_rest2, Snu_obs2, c='blue', alpha=0.6)
    ax7.set_xscale('log')
    ...
    ax8.plot(freq_rest, Snu_obs, c='orange')
    ax8.plot(freq_rest2, Snu_obs2, c='blue', alpha=0.6)
    ax8.set_xscale('log')
    ...
    ax9.plot(freq_rest, Snu_obs, c='orange')
    ax9.plot(freq_rest2, Snu_obs2, c='blue', alpha=0.6)
    ax9.set_xscale('log')
    ...
    #plt.show(block=True)
    fig.savefig('Plot_test_2.pdf')
    os.system('open Plot_test_2.pdf')
