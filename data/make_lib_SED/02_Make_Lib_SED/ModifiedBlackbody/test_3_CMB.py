...
def test():
    ...
    x_ref = x
    y_ref = y + y_CMB
    ax1.plot(x_CMB, y_CMB, ls='dashed', lw=1.0, c='blue', alpha=0.8, label=r'CMB at $z=%0.3f$'%(z))
    ax1.plot(x_ref, y_ref, ls='solid', lw=1.0, c='k', alpha=0.8, label=r'CMB + dust MBB')
    ...
    #plt.show(block=True)
    fig.savefig('Plot_MBB_test_3_CMB.pdf')
    os.system('open Plot_MBB_test_3_CMB.pdf')
