...
PRO makelibSED_BC03
    ; 
    restore, 'Zsolar_tau0.1Gyr_spec.save', /verbose
    SPEC_SFR = SFR
    SPEC_MASS = M_TOT
    SPEC_AGE = 10^LOGAGE
    ;
    outfile = 'lib.BC03.ExpSFH.tau0.1Gyr.Z0.0190.SED'
    ; 
    ...
    PRINTF, fp, FORMAT='(A,A)', "# ", "TPAR2 = tau         # the e-folding time scale of the exponential star formation history, in unit of Gyr"
    PRINTF, fp, FORMAT='(A,A)', "# ", "TPAR3 = Age         # in unit of Gyr"
    PRINTF, fp, FORMAT='(A,A)', "# ", "TPAR4 = Mass        # in unit of solar mass (associated with Age, not independent)"
    PRINTF, fp, FORMAT='(A,A)', "# ", "TPAR5 = SFR         # in unit of solar mass per year (associated with Age, not independent)"
    ...
    PRINTF, fp, FORMAT='(A,A,I0,A)', "# ", "NPAR3 = ", (SIZE(SPEC_FLUX,/DIM))[0], " # Number of different PAR3"
    PRINTF, fp, FORMAT='(A,A,I0,A)', "# ", "NPAR4 = ", 1, " # Number of different PAR4"
    PRINTF, fp, FORMAT='(A,A,I0,A)', "# ", "NPAR5 = ", 1, " # Number of different PAR4"
    PRINTF, fp, FORMAT='(A)', "# "
    PRINTF, fp, FORMAT='(A,A13,A15,A15,A12,A12,A15,A15)', "# ", "lambda", "flux", "metallicity", "tau", "age", "mass", "SFR"
    PRINTF, fp, FORMAT='(A,A13,A15,A15,A12,A12,A15,A15)', "# ", "[um]", "[Lsun/Hz]", "[---]", "[Gyr]", "[Gyr]", "[Msun]", "[Msun/yr]"
    ...
    FOR specindex = 0, (SIZE(SPEC_FLUX,/DIM))[0]-1 DO BEGIN
        ...
        PRINT, 'Processing spectrum ' + STRING(FORMAT='(I0)',specindex) + ' Z=' + ZMeta + ' tau=' + tau + ' age='+STRING(FORMAT='(E0.3)',SPEC_AGE[specindex])
        ...
        FOR i=0,N_ELEMENTS(flux)-1 DO BEGIN
            PRINTF, fp, FORMAT='(E15.6,E15.6,F15.5,F12.5,F12.5,E15.6,E15.6)', wave[i], flux[i], DOUBLE(ZMeta), DOUBLE(tau), age, mass, sfr
        ENDFOR
