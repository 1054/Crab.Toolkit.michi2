...
PRO makelibSED_BC03
    ...
    ; --
    ; about the flux unit
    ; the BC03 output flux has a unit of of solar luminosity per Hz
    ; and indicats that, with 'PAR3 (Mass)' amount of stars, we have that flux.
    ; so,  
    ; flux_BC03 [Lsun Hz-1] * PAR3 / (3.839e33*1e26/9.52140e48) * 4*pi*dL**2 / (1+z) --> makes the final stellar mass. 
    ; 
    ; So, we can make it simpler, by operating on the BC03 output flux: 
    ; Stellar mass
    ;   = flux_BC03 * 1e9 [Lsun GHz-1] * PAR3 / (3.839e33*1e26/9.52140e48) * 4*pi*dL**2 / (1+z)
    ;   = flux_BC03 * 1e9 / (3.839e33*1e26/9.52140e48) [Lsun GHz-1] * a * 4*pi*dL**2 / (1+z)
    ;   = flux_BC03 / 40.31970088 [Lsun GHz-1] * PAR3 * 4*pi*dL**2 / (1+z)
    ; 
    ...
    PRINTF, fp, FORMAT='(A,A)', "# ", "TVAR2 = flux density in unit of solar luminosity per Hz. flux * PAR3 / 40.31970088 * 4*pi*dL**2[Mpc**2] / (1+z) makes the total stellar mass."
    ...
    PRINTF, fp, FORMAT='(A,A)', "# ", "NVAR2 = 1    # NAXIS2 -- the number of spectra -- should equal to NPAR1*NPAR2 but only count for independent parameters."
