...
cd "SED_fitting_michi2" #<TODO># please adapt this directory name
...
for (( i = 0; i < ${#list_of_source_names[@]}; i++ )); do
    ...
    #if [[ ! -f ./$_Source/fit_5.pdf ]]; then continue; fi #<TODO>#<DEBUG>#
    ...
    cp ./$_Source/results_fit_5/fit_5.pdf             $output_dir/${_Source}_fit_5_components.pdf
    cp ./$_Source/results_fit_5/fit_5.best.pdf        $output_dir/${_Source}_fit_5_components.best.pdf
    cp ./$_Source/results_fit_5/fit_5.best.chisq.pdf  $output_dir/${_Source}_fit_5_components.chisq.pdf
    ...
    cp ./$_Source/results_fit_5/dump/1/SED_LIB[12345]   $output_dir/${_Source}_fit_5_component_SEDs/
    cp ./$_Source/results_fit_5/dump/1/SED_SUM          $output_dir/${_Source}_fit_5_component_SEDs/
    cp ./$_Source/fit_5.out.info                        $output_dir/${_Source}_fit_5_component_SEDs/info.txt
    ...
    for _Param in LIR_total Mdust_total Umean_total LAGN Mstar fPDR_total; do
        ...
        if [[ $i -eq 0 ]]; then
        head -n 1 ./$_Source/results_fit_5/best-fit_param_$_Param.txt  | sed -e "s/^/# Source${_SourceW1} /g" | sed -e "s/${_ParW1}param_/${_ParW2}${_Param}_/g"  >   $output_dir/best-fit_param_$_Param.txt
        fi
        tail -n 1 ./$_Source/results_fit_5/best-fit_param_$_Param.txt  | sed -e "s/^/$_Source${_SourceW2} /g"                                                     >>  $output_dir/best-fit_param_$_Param.txt
    ...
    # get chisq <20180216>
    if [[ $i -eq 0 ]]; then
    printf "# %s  %s\n" "Source${_SourceW1}" "chisq" > $output_dir/best-fit_chisq.txt
    printf "# %s  %s\n" "Source${_SourceW1}" "$(cat ./$_Source/SNR_FIRMM.txt | grep '^#' | head -n 1 | sed -e 's/#/ /g')" > $output_dir/best-fit_SNR_FIRMM.txt
    fi
    printf "%s  %s\n" "$_Source${_SourceW2}" "$(cat ./$_Source/results_fit_5/dump/1/chi2.txt | grep -v '^#' | head -n 1)" >> $output_dir/best-fit_chisq.txt
    printf "%s  %s\n" "$_Source${_SourceW2}" "$(cat ./$_Source/SNR_FIRMM.txt | grep -v '^#' | head -n 1)" >> $output_dir/best-fit_SNR_FIRMM.txt
    #cat ./$_Source/SNR_FIRMM.txt | grep -v '^#' | head -n 1 | sed -e "s/^/$_Source${_SourceW2}  /g" >> $output_dir/best-fit_SNR_FIRMM.txt
